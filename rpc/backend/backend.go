...
import (
	"context"
	"fmt"
	_ "github.com/tendermint/tendermint/state"
	"os"
...
//add func for blockmaze
//func (b *EthermintBackend) StateAndHeaderByNumber(blockNr rpc.BlockNumber)  {
...
//func(b *EthermintBackend) GetState() ([]byte, int64, error) {
//	res, height, err := b.clientCtx.QueryWithData(fmt.Sprintf("custom/%s/%s/%s", evmtypes.ModuleName, evmtypes.QueryTransactionLogs, common.BytesToHash(tx.Hash()).String()), nil)
